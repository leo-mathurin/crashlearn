...
| Bug 3 de `docs/scripted_driver.md` (E-11) : divergence en marche arrière braquée | `gym/f110_gym/envs/dynamic_models.py` | Le modèle cinématique est utilisé pour toute vitesse `< 0,5 m/s`, donc toute la marche arrière (le code livré testait `abs(v) < 0,5`). Le modèle single-track à pneus n'est valide qu'en marche avant : pour `v < 0`, son amortissement de lacet change de signe. Marche avant inchangée ; les 5 tests `f110_gym/unittest/dynamics_test.py` passent. | `test_steered_reverse_stays_stable` |
| Bug 5 de `docs/scripted_driver.md` (E-11) : pas de seed public | `env_simulation.py` | `set_seed(seed)` règle le RNG du simulateur (bruit LiDAR, tirages de friction) : il le réensemence tout de suite et pour chaque simulateur construit ensuite. Le seed par défaut reste 0. `reset()` ne réensemence toujours pas, pour que les épisodes successifs tirent un bruit différent. Remplace l'accès privé `_get_sim()._rng`. | `test_set_seed_makes_noise_and_friction_reproducible` |

## Remarques

- E-12 a corrigé D1, D4, D5 et D6 de l'audit E-10, ainsi que les bugs 1, 2, 3 et 5 relevés par le pilote de contrôle E-11 (tableau ci-dessus ; le bug 4 de cette liste est D1). D2 et D3 (schémas annoncés par `get_space_info` et par les observations factices d'`agent_loader`) ne sont **pas** corrigés ici : ils relèvent de notre adaptateur d'observations (E-14/E-16), et les modifier changerait ce que voit un agent au tournoi. La détection de carte du visualiseur et les métadonnées de replay restent à traiter avec les replays.
